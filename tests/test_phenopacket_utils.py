...
from phenopackets import (
    Family, Pedigree, Phenopacket, Individual, PhenotypicFeature, OntologyClass, File,
    MetaData, Resource, VcfRecord, GeneDescriptor, VariationDescriptor, VariantInterpretation, GenomicInterpretation,
...
from pheval.utils.phenopacket_utils import (
    PhenopacketUtil, CausativeVariant, IncompatibleGenomeAssemblyError,
...
proband = Phenopacket(
    ...
    phenotypic_features=[PhenotypicFeature(type=OntologyClass(id="HP:0000256", label="Macrocephaly")),
    ...
    interpretations=[Interpretation(id="test-subject-1-int", progress_status="SOLVED",
                                    diagnosis=Diagnosis(genomic_interpretations=[GenomicInterpretation(
                                        ...
                                        variant_interpretation=VariantInterpretation(
                                            ...
                                            variation_descriptor=VariationDescriptor(
                                                gene_context=GeneDescriptor(
                                                    value_id="HGNC:18654", symbol="RTTN"
                                                ),
                                                vcf_record=VcfRecord(
                                                    genome_assembly="GRCh37",
                                                    chrom="18",
...
phenopacket = Phenopacket(
    ...
    phenotypic_features=[PhenotypicFeature(type=OntologyClass(id="HP:0000256", label="Macrocephaly")),
                         ...
                         PhenotypicFeature(type=OntologyClass(id="HP:0003150", label="Glutaric aciduria")),
                         PhenotypicFeature(type=OntologyClass(id="HP:0001332", label="Dystonia")),
                         PhenotypicFeature(type=OntologyClass(id="HP:0008494", label="Inferior lens subluxation"),
                                           excluded=True)],
    interpretations=[Interpretation(id="test-subject-1-int", progress_status="SOLVED",
                                    diagnosis=Diagnosis(genomic_interpretations=[GenomicInterpretation(
                                        subject_or_biosample_id="test-subject-1",
    ...
    files=[File(uri="test/path/to/test_1.vcf", file_attributes={"fileFormat": "VCF", "genomeAssembly": "GRCh37"}, ),
           File(uri="test_1.ped", file_attributes={"fileFormat": "PED", "genomeAssembly": "GRCh37"}, )],
    meta_data=MetaData(
        created_by="pheval-converter",
        resources=[Resource(id="hp", name="human phenotype ontology", url="http://purl.obolibrary.org/obo/hp.owl",
...
family = Family(id="test-family-1",
                proband=proband,
                pedigree=Pedigree(persons=[Pedigree.Person(
...
class TestPhenopacketUtil(unittest.TestCase):
    ...
    def test_phenotypic_features(self):
        self.assertTrue(
            "HP:" in pheno_phenotypic_feature.type.id for pheno_phenotypic_feature in
            self.phenopacket.phenotypic_features())
        self.assertTrue(
            "HP:" in family_phenotypic_feature.type.id for family_phenotypic_feature in
            self.family.phenotypic_features())
    ...
    def test_causative_variants(self):
        self.assertEqual(len(self.phenopacket.causative_variants(Path("test-phenopacket-1.json"))), 2)
        self.assertFalse(self.phenopacket.causative_variants(Path("test-phenopacket-1.json"))[0] ==
                         self.phenopacket.causative_variants(Path("test-phenopacket-1.json"))[1])
        for causative_variant in self.phenopacket.causative_variants(Path("test-phenopacket-1.json")):
    ...
    def test_vcf_file_data(self):
        ...
        incorrect_genome_assembly = [
            File(uri="test/path/to/test_1.vcf", file_attributes={"fileFormat": "VCF", "genomeAssembly": "hg10"}, ) if
            file.file_attributes["fileFormat"] == "VCF" else file for file in phenopacket.files]
        ...
        with self.assertRaises(IncompatibleGenomeAssemblyError):
            PhenopacketUtil(incorrect_genome_assembly_ppacket).vcf_file_data(Path("test-phenopacket-1.json"),
                                                                             Path("input_dir"))
        incorrect_file_format_ppacket = copy.copy(phenopacket)
        del incorrect_file_format_ppacket.files[:]
        incorrect_file_format = [
            File(uri="test/path/to/test_1.ped", file_attributes={"fileFormat": "VCF", "genomeAssembly": "GRCh37"}, ) if
            file.file_attributes["fileFormat"] == "VCF" else file for file in phenopacket.files]
        incorrect_file_format_ppacket.files.extend(incorrect_file_format)
        with self.assertRaises(IncorrectFileFormatError):
            PhenopacketUtil(incorrect_file_format_ppacket).vcf_file_data(Path("test-phenopacket-1.json"),
                                                                         Path("input_dir"))

    def test_diagnosed_genes(self):
        self.assertEqual(len(self.phenopacket.diagnosed_genes()), 2)
        self.assertTrue(all(gene in ["RTTN", "FGD1"] for gene in self.phenopacket.diagnosed_genes()))
    ...
    def test_diagnosed_variants(self):
        ...
        for diagnosed_variant in self.phenopacket.diagnosed_variants():
            self.assertTrue(diagnosed_variant["geneSymbol"] == "RTTN" and diagnosed_variant[
                "variant"].genome_assembly == "GRCh37" and
                            diagnosed_variant["variant"].chrom == "18" and diagnosed_variant[
                                "variant"].pos == 67691994 and
                            diagnosed_variant["variant"].ref == "G" and diagnosed_variant["variant"].alt == "A" or
                            diagnosed_variant["geneSymbol"] == "FGD1" and diagnosed_variant[
                                "variant"].genome_assembly == "GRCh37" and
                            diagnosed_variant["variant"].chrom == "X" and diagnosed_variant[
                                "variant"].pos == 54492285 and
                            diagnosed_variant["variant"].ref == "C" and diagnosed_variant["variant"].alt == "T"
                            )
...
class TestPhenopacketRebuilder(unittest.TestCase):
    ...
    def test_add_randomised_hpo(self):
        ...
        self.family_rebuilder.add_randomised_hpo(
            [PhenotypicFeature(type=OntologyClass(id="HP:0000316", label="Hypertelorism")),
             PhenotypicFeature(type=OntologyClass(id="HP:RANDOM", label="RANDOM")), ])
        self.assertEqual(len(self.phenopacket_rebuilder.phenopacket_contents.phenotypic_features), 2)
        self.assertEqual(len(self.family_rebuilder.phenopacket_contents.proband.phenotypic_features), 2)
        for p_f in self.phenopacket_rebuilder.phenopacket_contents.phenotypic_features:
            self.assertTrue(
                p_f.type.id == "HP:0000316" and p_f.type.label == "Hypertelorism" or p_f.type.id == "HP:RANDOM"
                and p_f.type.label == "RANDOM")

    def test_add_created_vcf_path(self):
        self.phenopacket_rebuilder.add_created_vcf_path(Path("input_dir/test_vcf_dir/test_1.vcf"), "GRCh37")
        vcf_file = [file for file in self.phenopacket_rebuilder.phenopacket_contents.files if
                    file.file_attributes["fileFormat"] == "VCF"][0]
