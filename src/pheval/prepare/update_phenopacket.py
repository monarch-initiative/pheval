...
from pheval.utils.phenopacket_utils import (
    phenopacket_reader, PhenopacketUpdater, create_hgnc_dict,
...
def update_outdated_gene_context(phenopacket: Path, gene_identifier: str, hgnc_data: defaultdict):
    ...
    updated_phenopacket_contents = PhenopacketUpdater(phenopacket_contents,
                                                      hgnc_data, gene_identifier).update_phenopacket_interpretations()
    PhenopacketRebuilder(updated_phenopacket_contents).write_phenopacket(phenopacket)


@click.command()
@click.option("--phenopacket", "-p", metavar="FILE", required=True, help="Path to phenopacket for updating.", type=Path)
@click.option("--gene-identifier", "-g", required=False, default="ensembl_id", show_default=True,
              help="Gene identifier to add to phenopacket", type=click.Choice(['ensembl_id', 'entrez_id', 'hgnc_id']), )
...
@click.command()
@click.option("--phenopacket-dir", "-p", metavar="PATH", required=True,
              help="Path to phenopacket directory for updating.", type=Path)
@click.option("--gene-identifier", "-g", required=False, default="ensembl_id", show_default=True,
              help="Gene identifier to add to phenopacket", type=click.Choice(['ensembl_id', 'entrez_id', 'hgnc_id']), )
