...
class AssessGenePrioritisation:
    ...
    def _record_matched_gene(self, gene, rank_stats, standardised_gene_result):
        ...
        if float(self.threshold) == 0.0:
            return self._record_gene_prioritisation_match(gene, standardised_gene_result, rank_stats)
        else:
            return (
...
def generate_gene_stats_bar_plot(prioritisation_data: [TrackPrioritisation]) -> None:
    ...
    for prioritisation_result in prioritisation_data:
        gene_prioritisation_stats = _generate_stats_bar_plot_data(prioritisation_result, gene_prioritisation_stats,
                                                                  gene_analysis=True)
    gene_prioritisation_stats_df = pd.DataFrame(gene_prioritisation_stats)
    sns.catplot(data=gene_prioritisation_stats_df, kind="bar", x="Rank", y="Percentage", hue="Run").set(
        xlabel="Rank",
        ylabel="Ranked genes (%)",
    )
...
def generate_variant_stats_bar_plot(prioritisation_data: [TrackPrioritisation]) -> None:
    ...
    for prioritisation_result in prioritisation_data:
        variant_prioritisation_stats = _generate_stats_bar_plot_data(prioritisation_result,
                                                                     variant_prioritisation_stats, gene_analysis=False)
    variant_prioritisation_stats_df = pd.DataFrame(variant_prioritisation_stats)
    sns.catplot(data=variant_prioritisation_stats_df, kind="bar", x="Rank", y="Percentage", hue="Run").set(
        xlabel="Rank",
...
def generate_benchmark_variant_output(prioritisation_data: TrackPrioritisation) -> None:
    RankComparisonGenerator(prioritisation_data.variant_prioritisation.ranks).generate_variant_output(
        f"{prioritisation_data.gene_prioritisation.results_dir.name}"
    )
...
def generate_benchmark_comparison_gene_output(prioritisation_stats_for_runs: [TrackPrioritisation]) -> None:
...
def generate_benchmark_comparison_variant_output(prioritisation_stats_for_runs: [TrackPrioritisation]) -> None:
...
def benchmark_runs(
    ...
    generate_benchmark_comparison_gene_output(prioritisation_stats_for_runs) if gene_analysis else None
    generate_benchmark_comparison_variant_output(prioritisation_stats_for_runs) if variant_analysis else None
    gene_stats_writer.close() if gene_analysis else None
    variants_stats_writer.close() if variant_analysis else None
